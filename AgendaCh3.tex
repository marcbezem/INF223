...
also denoted $(A_i \mid i\in I)$; we write $A(i)=A_i$
\item We can promote any set $I$ to a graph without edges: 
$(I,\emptyset,in_{\emptyset,I},in_{\emptyset,I})$, also denoted I (abusively)
 \item Then an $I$-indexed family of sets is an interpretation $A$ in $[I\to \Set]$
...
L\arrow{r}{ar}\&R
\end{tikzcd}
\item $Ar$ is almost its path category, just add identities
...
\item natural transf.s $\alpha: f\To g$ between such interpretations, being
morphisms $\alpha_L: f(L)\To g(L)$ and $\alpha_R: f(R)\To g(R)$ such that
the naturality square holds for $ar: L\to R$: $f(ar);\alpha_L = \alpha_R;f(ar)$
   \end{itemize}
\item The interpretation category $[Ar\to\CC]$ and the functor category
$[{\cdot}{\to}{\cdot}\to\CC]$ are isomorphic
\item Uhh, what's this? Well, let's say small $\CC$, so in $\Cat$
