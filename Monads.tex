...
slide \ref{Mon5:PowSetMonad}, where the functor is $\powset_{fin}$ on sets,
and extended to functions $f:A\to B$ by setting $\powset_{fin}(f)(X) = f(X)$,
and this image is indeed in $\powset_{fin}(B)$ for $X\in\powset_{fin}(A)$.
