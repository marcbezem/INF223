...
\item Examples: $[\set{|}] \simeq (\nat,0,{+})$, 
$(\powset{\set{0}},\emptyset,{\cup}) \simeq (\set{0,1},0,{\max})$,
$(\powset{\set{0}},\emptyset,{\cap}) \simeq (\set{0,1},0,{\min})$
...
$F=(F_V,F_E): gr(\CC)\to gr(\DD)$ satisfying
   \begin{itemize}[<+->]
\item $F_E(f;^\CC g) = F_E(f);^\DD F(g)$ for all objects $A,B,C$ of $\CC$ 
...
\item $map~id_T = id_{[T]}$
   \end{itemize}
\item Later: e.g., reversing lists is a natural transformation, and can therefore be
