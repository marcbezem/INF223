...
\frame
 ...
 \begin{itemize}[<+->]
\item A \emph{diagram (in $H,\CC$) of shape} $I$ is a graph homomorphism
$\delta : I\to H$, if $H = gr(\CC)$ we simply write $\delta : I\to \CC$

\item A diagram $\delta : I\to \CC$ is \emph{commutative} if the
extension $\delta^* : P(I) \to gr(\CC)$ has the following property:
for any paths $p_1,p_2 : v\to u$ in $P(I)$, we have $\delta^*(p_1)=\delta^*(p_2)$
\item This is more tricky than it seems:
   \begin{enumerate}[<+->]
\item the equality $\delta^*(p_1)=\delta^*(p_2)$ is in $\CC(\delta(u),\delta(v))$
\item there are \emph{always} infinitely many paths since we can insert empty
paths in the lists, but they change nothing (by ID)
\item there can be loops, but we only have to check them once
\item there can be (even hidden) empty paths that are crucial
\item the commutativity of a finite diagram reduces \emph{always}
to finitely many cases
   \end{enumerate}
\item Example of 4 above, isomorphy:
\begin{tikzcd}[ampersand replacement=\&]
%1 \arrow[yshift=1ex]{r}{y} \& 2 \arrow[yshift=-1ex]{l}{z}  \& \&
