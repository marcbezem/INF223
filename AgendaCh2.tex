...
\frame
  {
    ...
    \frametitle{Relation Therapy (after MNF130)}\label{Ch2:BinRels}
...
\frame
  {
    ...
    \frametitle{Relation Therapy (ctnd)}\label{Ch2:BinRelsCtnd}

 \begin{itemize}[<+->]\label{right-left-invariance}
    \item Examples
    \begin{itemize}[<+->]
    \item total order: $\leq$ on sets of numbers
    \item partial order: $\subseteq$ on sets % often on the set $\powset S$
    \item preorder: $\sqsubseteq$ on persons defined by age comparison
...
explaning why $R$ is called preorder. Age classes are even totally ordered.
 \end{itemize}
