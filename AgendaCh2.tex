...
defined by $\lambda_R(b) = \set{a\in B\mid R(a,b)}$
\end{itemize}
...
\item between $\powset(A\times B)$ and $B\to \powset{A}$: $R_{\lambda_R} = R^c$ and $\lambda_{R_g} = g$
 \end{itemize}
\item The catergory $\Mult$ has sets as objects and multimaps as morphisms,
...
recall $aR_fb = (b\in f(a))$ and $bR_g c = (c\in f(b))$
...
such that $(v,u)\in G_E$ implies $(\varphi(v),\varphi(u))\in H_E$
...
such that $id^G(A): A \to A$ for all $A\in G_V$
\item Homomorphism $\varphi: G\to H$ of ordinary graphs: map $\varphi: G_V\to H_V$
such that $(v,u)\in G_E$ implies $(\varphi(v),\varphi(u))\in H_E$
\item Tempting but wrong: an iso is a bijective homomorphism.
\item Wrong since $H$ can have more arrows than $G$
