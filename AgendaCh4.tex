...
\begin{itemize}
 ...
 \end{itemize}
They are each other's dual, and we do the diagrams first %(next slide)
\item Seen already (not in an arbitrary category):
\begin{itemize}
...
\[
\begin{tikzcd}[ampersand replacement=\&]
A \arrow{r}{\kappa_1} \arrow[swap]{dr}{f_x}\&
C\arrow{d}[description]{\exists!k} \& 
B \arrow[swap]{l}{\kappa_2}\arrow{dl}{f_y} \&
A \&
C\arrow[swap]{l}{\pi_1} \arrow{r}{\pi_2}\&
B\\
\& X\&\& \&
X\arrow{ul}{f_x}\arrow{u}[description]{\exists!k} \arrow[swap]{ur}{f_y}
\end{tikzcd}
\]
\item Sum in $\CC$: a co-span $A\stackrel{\kappa_1}{\to} C \stackrel{\kappa_2}{\leftarrow}B$ 
is called a \emph{sum} of $A$ and $B$ 
if for any co-span 
$A\stackrel{f_x}{\to} X \stackrel{f_y}{\leftarrow}B$ 
there is a unique morphism $k: C\to X$
such that $f_x = \kappa_1;k$ and $f_y = \kappa_2;k$
...
$A\stackrel{f_x}{\leftarrow} X \stackrel{f_y}{\to}B$ 
there is a unique morphism $k: X\to C$
such that $f_x = k;\pi_1$ and $f_y = k;\pi_2$
...
$A\stackrel{\kappa_1}{\to} A\uplus B \stackrel{\kappa_2}{\leftarrow}B$,
with (relation)
$\kappa_1 = \set{(a,(a,1))\mid a\in A}$ and $\kappa_2 = ...$  has the universal property
\begin{tikzcd}[ampersand replacement=\&]
A \arrow{r}{\kappa_1} \arrow[swap]{dr}{R_1}\&
A\uplus B\arrow{d}[description]{\exists!K} \& 
B \arrow[swap]{l}{\kappa_2}\arrow{dl}{R_2} \\
\& X\&
\end{tikzcd}
...
\\\vspace*{0.2cm}
This example show how important the morphisms are: $\Incl$
and $\Set$ have different morphisms, and even though they have 
the same objects, the sums are different.
\\\vspace*{0.2cm}
...
$2+2=2$, but no sum $1+2$ (no lub of $1$ and $2$).

 }

\frame
  {   
    \frametitle{Uniqueness of sums}\label{Ch4:SumUnique}

 %\begin{itemize}[<+->]
%\item 
If also the co-span $A\stackrel{f_x}{\to} X \stackrel{f_y}{\leftarrow}B$
is a sum we have:
\[
\begin{tikzcd}[ampersand replacement=\&]
A \arrow{r}{\kappa_1} \arrow[swap]{dr}{f_x}\&
...
A \arrow{r}{\kappa_1} \arrow[swap]{dr}{f_x} \arrow{ur}{f_x}\&
C\arrow{d}[description]{k} \& 
B \arrow[swap]{l}{\kappa_2}\arrow{dl}{f_y} \arrow[swap]{ul}{f_y}\&
A \arrow{ru}{\kappa_1} \arrow{r}{f_x} \arrow[swap]{dr}{\kappa_1}\&
X \arrow{d}[description]{j}\& 
B \arrow[swap]{l}{f_y}\arrow{dl}{\kappa_2} \arrow[swap]{ul}{\kappa_2}\\
\& X\&\& \&
