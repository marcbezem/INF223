...
\frame
 ...
 \begin{itemize}[<+->]
\item Monomorphism is the categorical version of an injective map in $\Set$
\item In a given a category $\CC$ and objects $A,B$, a morphism $f: A\to B$ is a
\emph{monomorphism} or \emph{mono} if for all objects $X$ and 
morphisms $g,h: X\to A$ we have $g=h$ if $g;f = h;f$ %(`right cancellation')
\item In diagrams: if
...
\item In $\Set$ this is equivalent with $f$ being injective:
 \begin{itemize}
    \item if $f$ injective, then $f(g(x))=f(h(x)$ implies $g(x)=h(x)$ for all $x\in X$
    \item if $f$ mono, then ...
 \end{itemize}
