...
where $\pi_1 = (\pi_{1,V},\pi_{1,E}) : G\times H \to G$ and $\pi_2: G\times H \to$
...
\frame
 ...
 \begin{itemize}[<+->]
\item Recall the definition of the kernel of $f: A\to B$:
...
\item Also, $graph(f) = \set{(a,b) \in A\times B \mid f(a)=b}$ (right)
...
(the last $C$ can be replaced by $f(A)\cap f(B)$)
...
\frame
  {   
    \frametitle{Pullback = product + equalizer}\label{Ch4:PullbProdEq}

 \begin{itemize}[<+->]
\item Given as span $A\stackrel{f}{\to} C\stackrel{g}{\leftarrow}B$ in 
a category $C$ with products, we get a span
$A\times B\stackrel{\pi_1;f}{\to} C\stackrel{\pi_2;g}{\leftarrow}A\times B$
of two parallel morphisms
\item The pullback can now be obtained if $\CC$ also has equalizers:
\item Given to parallel morphisms $f,g: A\to B$, an \emph{equalizer}
consists of an object $E$ and a morphism $m:E\to A$ with $m;f=m;g$
such that for all objects $X$ and morphism $h:X\to A$ with $h;f=h;g$
