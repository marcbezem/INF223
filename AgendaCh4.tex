...
B \arrow{l}{\kappa_2}\arrow[swap]{dl}{f_2} \arrow{ddl}{f_2;g}\\
...
where $f_1+f_2 : A_1\uplus B_1 \to B_1+B_2$ is defined case-wise.\\\vspace*{0.3cm}
...
$\CC/T$ has sums: $(A\uplus B, [f_1,f_2])$ is an object of $\CC/T$,
...
The unique morphisms $k:  X\to A\times B$ in the universal property are
...
\\\vspace*{0.2cm}
We could have known better: $\Rel\simeq\Rel^{op}$ with an iso that 
is the identity on objects. Sums and products are dual, so the objects
of sum and product in $\Rel$ are the same, $A\uplus B$. A detailed elabora- tion
of the product in $\Rel$ based on dualisation is in the script.
\\\vspace*{0.2cm}
In the category $\Gra$, products use $\times$ of $\Set$ as expected.
...
How to type `mixed pairs'  $(1,\pi)$, $(\pi,1)$ ? Throw them out!
...
such that $f;h_1 = f;h_2$
there is a unique morphism $k: X\to C$
such that $h_i = k;\pi_i$. % and $h_2 = k;\pi_2$. 
In a diagram:
\begin{tikzcd}[ampersand replacement=\&]
...
\frame
 ...
 \begin{itemize}[<+->]
\item If $A\stackrel{f}{\leftarrow} C \stackrel{g}{\to}B$ in $\Set$,
...
C 
\end{tikzcd}
\item If $\CC$ has a terminal 1 and pullbacks, then $A\times_1 B$ is a product
\item If such a pullback exists, object $D$ is denoted by $A\times_C B$ 
% (up to unique isomorphism) 
and the mediating morphism $k$ by $\langle f,g\rangle_C$
